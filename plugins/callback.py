...
async def button(bot, update):
    try:
        logger.info(f"Callback received: {update.data[:50]}")
        
        # Language callback
        if update.data.startswith("set_lang_"):
            from plugins.commands import language_callback
            await language_callback(bot, update)
            return
