...
class TorrentManager:
    ...
    async def _monitor_torrent(self, info_hash: str, user_id: int = 0):
        ...
        if info_hash not in self.active_torrents:
            logger.error(f"Torrent {info_hash} not found in active torrents")
            return
        ...
        logger.info(f"Starting to monitor torrent: {info_hash}")
        ...
        try:
            ...
            while info_hash in self.active_torrents:
                try:
                    ...
                    if check_count % 10 == 0:
                        logger.info(f"Torrent {info_hash}: {self._get_state_name(status.state)} - {status.progress * 100:.1f}%")
                    ...
                    if status.has_metadata and filename == "Fetching metadata...":
                        ...
                        logger.info(f"Torrent metadata fetched: {filename}")
                            ...
                            try:
                                ...
                                for i in range(torrent_info.num_files()):
                                    try:
                                        ...
                                        logger.warning(f"Failed to get file path for file {i}: {e}")
                                        # Fallback
                                        file_path = os.path.join(
                                            torrent_data['save_path'],
                                            f"file_{i}"
                                ...
                                logger.error(f"Failed to get files from torrent: {e}")
                    ...
                    if status.state == lt.torrent_status.checking_resume_data:
                        logger.warning(f"Torrent checking resume data: {info_hash}")
            ...
            logger.error(f"Monitor torrent error: {e}", exc_info=True)
