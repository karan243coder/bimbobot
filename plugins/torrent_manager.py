...
class TorrentManager:
    ...
    def _init_session(self):
        """Initialize libtorrent session with optimized settings for speed"""
        try:
            settings = {
                ...
                'connections_limit': 1000,  # Increased from 500
                'max_peerlist_size': 2000,  # Increased from 1000
                'max_paused_peerlist_size': 1000,  # Increased from 500
                'seed_time_limit': 0,  # No seed time limit
                'auto_manage_startup': 10,  # Increased from 5
                'active_downloads': 20,  # Increased from 10
                'active_seeds': 20,  # Increased from 10
                'active_limit': 40,  # Increased from 20
                # Additional speed optimizations (all integers)
                'connection_speed': 100,  # Faster connection attempts
                'max_out_request_queue': 1500,  # Increased request queue
                'request_queue_size': 10,  # Increased request queue per peer
                'whole_seeders_limit': 50,  # More whole seeders
                'mixed_mode_algorithm': 1,  # Prefer TCP
                'rate_limit_ip_overhead': True,
                'announce_to_all_trackers': True,  # Announce to all trackers
                'announce_to_all_tiers': True,  # Announce to all tiers
                'prefer_udp_trackers': True,  # Prefer UDP trackers (faster)
                'max_failcount': 3,  # Allow more failures before giving up
                'min_announce_interval': 30,  # More frequent announces
            }
            
            self.session = lt.session(settings)
            logger.info("Torrent session initialized with optimized speed settings")
            ...
            try:
                ...
                self.session = lt.session(basic_settings)
                logger.info("Torrent session initialized with basic settings (fallback)")
            except Exception as e2:
                logger.error(f"Failed to initialize torrent session with basic settings: {e2}")
