...
def build_terabox_keyboard(tb_info):
    ...
    if is_video:
        ...
        inline_keyboard.append([
            InlineKeyboardButton(f"🎬 Send as Video ({size_text})", callback_data="terabox=video".encode("UTF-8")),
            InlineKeyboardButton(f"📁 Send as File ({size_text})", callback_data="terabox=file".encode("UTF-8")),
        ])
    elif is_audio:
        # Audio file
        inline_keyboard.append([
            InlineKeyboardButton(f"🎵 Send as Audio ({size_text})", callback_data="terabox=audio".encode("UTF-8")),
            InlineKeyboardButton(f"📁 Send as File ({size_text})", callback_data="terabox=file".encode("UTF-8")),
        ])
    else:
...
async def echo(bot, update):
    ...
    if is_terabox(url):
        ...
        if tb_info and tb_info.get("download_url"):
            ...
            reply_markup = build_terabox_keyboard(tb_info)
