...
async def cancel_torrent_callback(client: Client, callback_query):
    ...
    if info_hash in torrent_manager.active_torrents:
        ...
        if torrent_data.get('user_id') == user_id:
            try:
                ...
                asyncio.create_task(auto_delete_messages(None, callback_query.message, 15))
                
                await callback_query.answer("✅ Torrent cancelled successfully!", show_alert=True)
            except Exception as e:
