...
# ======================= CANCEL TORRENT CALLBACK =======================
@Client.on_callback_query(filters.regex(r"^cancel_torrent_"))
async def cancel_torrent_callback(client: Client, callback_query):
    """Handle cancel torrent button click"""
    try:
        user_id = callback_query.from_user.id
        
        # Extract info_hash from callback data
        info_hash = callback_query.data.replace("cancel_torrent_", "")
        
        # Get torrent manager
        from plugins.torrent_manager import torrent_manager
        
        # Answer the callback immediately
        await callback_query.answer("⏳ Cancelling torrent...", show_alert=False)
        
        # Check if torrent exists
        if info_hash not in torrent_manager.active_torrents:
            await callback_query.answer("❌ Torrent not found or already completed!", show_alert=True)
            # Try to remove the button from message if it exists
        ...
        torrent_data = torrent_manager.active_torrents[info_hash]
        
        # Check if torrent belongs to this user
        if torrent_data.get('user_id') != user_id:
            await callback_query.answer("❌ This torrent doesn't belong to you!", show_alert=True)
            return
        ...
        try:
            ...
            # Try to edit message to show cancelled
            if callback_query.message:
                try:
                    await callback_query.message.edit_text(
                        "❌ **Torrent Cancelled!**\n\n"
                        "Download has been cancelled and files deleted.",
                        reply_markup=None
                    )
                    ...
                    try:
                        ...
                        logger.warning(f"Could not send cancel message: {e2}")
            else:
                # Message is None, send new message
                try:
                    await client.send_message(
                        chat_id=user_id,
                        text="❌ **Torrent Cancelled!**\n\nDownload has been cancelled and files deleted."
                    ...
                    logger.warning(f"Could not send cancel message: {e}")
            
        except Exception as e:
            logger.error(f"Cancel torrent error: {e}", exc_info=True)
            await callback_query.answer(f"❌ Error cancelling: {str(e)[:50]}", show_alert=True)
        ...
        try:
            ...
            if data.get('is_finished'):
                ...
                cancel_markup = InlineKeyboardMarkup([
                    [InlineKeyboardButton("❌ Cancel Download", callback_data=f"cancel_torrent_{info_hash}")]
                ])
                
                await progress_msg.edit_text(progress_text, reply_markup=cancel_markup)
