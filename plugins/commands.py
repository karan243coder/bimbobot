...
async def torrent_command(client: Client, message: Message):
    ...
    from helper_funcs.display_progress import humanbytes, TimeFormatter, build_progress_bar
        ...
        if info_hash:
            ...
            while True:
                ...
                if status.get('is_finished'):
                    ...
                    if torrent_data and 'files' in str(torrent_data):
                        # Files are available, now upload them
                        break
                await asyncio.sleep(2)
            
            # Get torrent status to get file info
            status = await torrent_manager.get_torrent_status(info_hash)
            if status and status.get('is_finished'):
                ...
                if torrent_data:
                    ...
                    if torrent_info:
                        ...
                        if largest_file and os.path.exists(largest_file):
                            # Upload the file
                            file_name = os.path.basename(largest_file)
                            ...
                            if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                                # Video file
                                try:
                                    width, height, duration = await Mdata01(largest_file)
                        ...
                        await progress_msg.edit_text("❌ Could not get torrent info!")
                        await asyncio.sleep(15)
                        await progress_msg.delete()
                else:
                ...
                await progress_msg.edit_text("❌ Torrent download failed!")
                await asyncio.sleep(15)
                await progress_msg.delete()
        else:
            await progress_msg.edit_text("❌ Failed to add torrent!")
            await asyncio.sleep(15)
            await progress_msg.delete()
    except Exception as e:
