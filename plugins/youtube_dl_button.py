            ...
            try:
                if now - last_progress_update >= PROGRESS_UPDATE_INTERVAL:
                    ...
                    if total_bytes > 0:
                        ...
                        # Update unified progress tracker for advanced UI
                        from helper_funcs.display_progress import update_task, update_user_progress
                        speed_bytes = 0
                        try:
                            # Parse speed like "2.50MiB/s" to bytes
                            speed_num = re.search(r'([\d\.]+)', speed)
                            if speed_num:
                                speed_val = float(speed_num.group(1))
                                if 'GiB' in speed or 'GB' in speed:
                                    speed_bytes = speed_val * 1024 * 1024 * 1024
                                elif 'MiB' in speed or 'MB' in speed:
                                    speed_bytes = speed_val * 1024 * 1024
                                elif 'KiB' in speed or 'KB' in speed:
                                    speed_bytes = speed_val * 1024
                                else:
                                    speed_bytes = speed_val
                        except:
                            speed_bytes = 0
                        
                        update_task(progress_task_id, downloaded_bytes, total_bytes, speed_bytes, 'downloading', 'yt-dlp')
                        await update_user_progress(None, update.from_user.id)
                    else:
                        # Fallback to simple card if size unknown
