...
async def split_large_file(file_path, max_size_bytes=1900000000, progress_msg=None):
    ...
    if progress_msg:
        ...
        await safe_edit(progress_msg, split_text)
    
    # For video files, split by duration
    try:
    ...
    # Fallback: Split by size using dd command (for non-video files)
    for i in range(num_parts):
        output_file = os.path.join(output_dir, f"{base_name}_part{i+1:02d}{ext}")
...
async def youtube_dl_call_back(bot, update):
    ...
    if needs_split:
        ...
        # Upload all parts as media group
        try:
            ...
            logger.error(f"Media group upload error: {e}")
    ...
    try:
        ...
        # DELETE the upload progress message
        try:
            await progress_msg.delete()
        except Exception as e:
            logger.warning(f"Could not delete progress message: {e}")
        
        # Send NEW compact success message
        upload_duration = (datetime.now() - start).seconds
        minutes, seconds = divmod(upload_duration, 60)
        time_str = f"{minutes}m {seconds}s" if minutes > 0 else f"{seconds}s"
        ...
        success_text = (
            ...
            "╰─────────────────────────────╯\n"
            f"📁 {escape_html(trim_text(file_name, 40))}\n"
            f"📦 {humanbytes(file_size)} • ⏱️ {time_str} • 🎬 {tg_send_type.upper()}\n\n"
            "🔗 @Bimbobot69"
        )
