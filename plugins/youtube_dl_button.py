...
async def youtube_dl_call_back(bot, update):
    ...
    try:
        ...
        update_task(progress_task_id, file_size, file_size, 0, 'completed', 'pyrogram')
        
        # Remove the task from tracker to stop progress updates
        remove_task(progress_task_id)
        
        # Show success message IMMEDIATELY (before log upload)
        success_text = (
            "╔════════════════════════════════════╗\n"
            "║      ✅ **UPLOAD COMPLETE**         ║\n"
            "╚════════════════════════════════════╝\n\n"
            f"📁 **File:** <code>{escape_html(trim_text(file_name, 60))}</code>\n"
            f"📦 **Size:** {humanbytes(file_size)}\n"
            f"⏱️ **Time:** {(datetime.now() - start).seconds}s\n\n"
            "<b>🎉 Successfully uploaded to Telegram!</b>\n\n"
            "<b>Join:</b> @Bimbobot69"
        )
        try:
            ...
            logger.warning(f"Could not edit success message: {e}")
